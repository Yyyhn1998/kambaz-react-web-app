...
import { fetchQuizById, fetchStudentQuizAttempts } from "./client";
...
export default function QuizResultPage() {
    ...
    useEffect(() => {
        const loadQuizAndAttempts = async () => {
            ...
            try {
                const quizData = await fetchQuizById(qid);
                setQuiz(quizData);

                try {
                    const attemptsData = await fetchStudentQuizAttempts(qid, currentUser._id);
                    ...
                    <Card key={question._id} className={`mb-4 ${correct ? 'border-success' : 'border-danger'}`}>
                        ...
                        <Card.Body className="py-4">
                            ...
                            <div className="mb-4 fs-5">
                                {question.text.replace(/\[blank\]/g, "____________")}
                            </div>
