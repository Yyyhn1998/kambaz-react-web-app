...
import { fetchQuizById, submitQuizAttempt, fetchStudentQuizAttempts } from "./client";
...
export default function QuizTakePage() {
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            await submitQuizAttempt(qid, submission);
    ...
    useEffect(() => {
        const loadPreviousAttempts = async () => {
            if (qid && currentUser?._id) {
                try {
                    const attempts = await fetchStudentQuizAttempts(qid, currentUser._id);
                                ...
                                {question.choices.map((choice: string, cidx: number) => (
                                    <div key={cidx}>
                                        <div className="d-flex align-items-center mb-2">
                                            <Form.Check
                                                type="radio"
                                                id={`question-${questionId}-choice-${cidx}`}
                                        ...
                                        {cidx < question.choices.length - 1 && (
        ...
        <Container className="py-4">
            <Card className="mb-4">
                <Card.Body>
                    ...
                    {quiz.description && <p className="text-muted mt-3">{quiz.description}</p>}
                </Card.Body>
            </Card>
